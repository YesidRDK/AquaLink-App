// screens/cliente/MapPickerScreen.js
      ...
      {!currentLocation ? (
        ...
        <View style={{ flex: 1 }}>
          <WebView
            ...
            source={{ html: `
              <html>
                <head>
                  ...
                  <style> body { margin: 0; } #map { height: 100vh; width: 100vw; } .leaflet-marker-icon { filter: hue-rotate(140deg); } </style>
                </head>
                <body>
                  <div id="map"></div>
                  <script>
                    ...
                    var userCircle = L.circleMarker([userLat, userLng], { radius: 10, fillColor: "#1976D2", color: "#fff", weight: 3, opacity: 1, fillOpacity: 0.8 }).addTo(map);
                    userCircle.bindPopup("<b>Estás aquí</b>").openPopup();
                    var deliveryMarker = L.marker([userLat, userLng], { draggable: true, autoPan: true }).addTo(map);
            ...
            style={{ flex: 1 }}
          />
          <View style={{ paddingHorizontal: 20, paddingTop: 20, paddingBottom: 65, backgroundColor: '#fff', elevation: 10 }}>
            ...
            <Text style={{ textAlign: 'center', marginBottom: 15, color: '#555' }}>El círculo azul es tu guía. Arrastra el pin a donde deseas el agua.</Text>
            
            <TouchableOpacity style={[styles.mainButton, { flexDirection: 'row', justifyContent: 'center', alignItems: 'center' }]} onPress={confirmarUbicacion}>
            ...
            <TouchableOpacity style={{ marginTop: 15, flexDirection: 'row', justifyContent: 'center', alignItems: 'center' }} onPress={() => navigation.goBack()}>
